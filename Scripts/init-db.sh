...
until pg_isready -h postgres -U trading_user -d trading_db; do
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    CREATE INDEX IF NOT EXISTS idx_trading_events_user_id ON trading_events(user_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_order_executions_timestamp ON order_executions(timestamp);
    ...
    CREATE INDEX IF NOT EXISTS idx_trading_analytics_symbol ON trading_analytics(symbol);
    CREATE INDEX IF NOT EXISTS idx_trading_analytics_timestamp ON trading_analytics(timestamp);
